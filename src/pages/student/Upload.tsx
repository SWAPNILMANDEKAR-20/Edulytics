...
export default function StudentUpload() {
  ...
  useEffect(() => {
    ...
    const fetchExams = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          // Filter to only confirmed exams
          const confirmed = data.filter((e: any) => 
            e.is_answer_key_confirmed === true && 
            (e.evaluation_type === 'descriptive' || !e.evaluation_type)
          );
          setExams(confirmed);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedExamId) {
      alert('Cannot upload script: No confirmed exam is available for this course subject.');
      return;
    }
    if (e.target.files) {
          ...
          setTimeout(async () => {
            try {
              ...
              if (ocrRes.ok) {
                ...
                const joinedText = ocrData.boundingBoxes
                  ? ocrData.boundingBoxes.map((b: any) => b.text).join(' ')
                  : 'No text extracted.';
                setOcrText(joinedText);
              }
            } catch (err) {
  ...
  const runAIEvaluation = async () => {
    ...
    try {
      ...
      if (res.ok) {
        if (data.warning === 'AI_SCORING_FAILED') {
          alert(`Warning: ${data.message || 'AI evaluation failed, but your sheet has been saved for manual grading by the professor.'}`);
        }
        setTimeout(() => {
        ...
        console.error('AI evaluation failed details:', data);
        alert(`AI evaluation engine failed: ${data.message || 'Unknown Server Error'} (${data.error || '500'})`);
        setCurrentStep(2);
        setIsProcessing(false);
      }
    } catch (err: any) {
      console.error('Evaluation failed:', err);
      alert(`AI evaluation failed: ${err.message || 'Network Error'}`);
      setCurrentStep(2);
      setIsProcessing(false);
    }
  };
    ...
    <div className="flex flex-col gap-8 pb-10">
      ...
      <div className="flex justify-between items-start flex-col gap-2">
        ...
        <p className="text-sm text-gray-400 max-w-2xl">
          Submit scans of your exam papers or OMR bubble sheets to trigger SBERT similarity OCR grading pipelines and receive feedback.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto w-full">
        ...
        {currentStep === 1 && (
          <div className="glass-card p-8 rounded-card border border-white/10 bg-[#0F1424]/40 flex flex-col items-center justify-center text-center gap-6 min-h-[350px]">
            <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 hover:border-brand-blue hover:text-brand-blue transition-colors">
            ...
            <div className="space-y-4 w-full flex flex-col items-center">
              <div>
                <h4 className="text-md font-bold text-white mb-2">
                  {exams.find(e => e.id === selectedExamId)?.evaluation_type === 'omr' 
                    ? 'Upload Scanned OMR Bubble Sheet' 
                ...
                <p className="text-xs text-gray-500 max-w-sm leading-relaxed font-semibold">
                  {exams.find(e => e.id === selectedExamId)?.evaluation_type === 'omr'
                    ...
                    : 'Submit raw scanned exams (PDF, PNG, JPG). Our OCR algorithm transcription parses handwritten texts automatically.'
                  }
                </p>
              </div>
              ...
              <div className="w-full max-w-xs space-y-1.5 text-left">
                ...
                {exams.length > 0 ? (
                  ...
                  <div className="text-[10px] text-status-error font-bold bg-status-error/10 border border-status-error/20 p-2.5 rounded-xl">
                    No confirmed exams found. Submissions are closed until the professor confirms the answer key.
                  </div>
                )}
              </div>
            </div>
              ...
              {selectedFiles.length > 0 && (
                <div className="text-left space-y-2 bg-[#0A0D1A]/60 border border-white/5 rounded-xl p-4 w-full">
                  ...
                  >
                    ...
                    <span>Upload & Process Script</span>
                  </button>
                </div>
              )}
            </div>
        ...
        {/* Step 2: OCR transcription status */}
        {currentStep === 2 && (
          <div className="glass-card p-8 rounded-card border border-white/10 bg-[#0F1424]/40 flex flex-col gap-6 min-h-[320px]">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              <h4 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                ...
                <span>OCR transcription complete</span>
              </h4>
            </div>

            <div className="space-y-2">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Scanned Text Preview</span>
            ...
            >
              ...
              <span>{isProcessing ? 'AI evaluating...' : 'Submit to AI Grading Engine'}</span>
            </button>
          </div>
        )}
        ...
        {currentStep === 3 && (
          <div className="glass-card p-8 rounded-card border border-white/10 bg-[#0F1424]/40 flex flex-col items-center justify-center text-center gap-6 min-h-[320px]">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 max-w-sm leading-relaxed font-semibold">
                SBERT vector semantic similarity computations and linear regression score estimations are running. Please wait...
              </p>
            </div>
          </div>
        )}
        ...
        {currentStep === 4 && (
          <div className="glass-card p-8 rounded-card border border-white/10 bg-[#0F1424]/40 flex flex-col items-center justify-center text-center gap-6 min-h-[320px] animate-fadeIn">
            ...
            <div>
              <h4 className="text-md font-bold text-white mb-2">Evaluation Completed!</h4>
              <p className="text-xs text-gray-500 max-w-sm leading-relaxed font-semibold">
                Your grades, SBERT scores, matched criteria checklist, and feedback suggestions are compiled.
              </p>
            </div>
